...
class AuditionAPITestCase(APITestCase):
    def setUp(self):
        ...
        TopicService(user=self.user, sphere=LearningSpheres.AUDITION).set_topic(
            topic_id=None
        )
    ...
    def test_sections_list(self):
        ...
        response = self.client.get(reverse("audition:sections_list"), format="json")
    ...
    def test_retrieve_current_topic(self):
        token = get_token_for_user(self.user)
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {token}")
        response = self.client.get(reverse("audition:topic"), format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["topic"], self.audition_topics[0].id)
